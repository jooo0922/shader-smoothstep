...
/*
  위에 코드 분석한 코멘트와 필기들을 읽어보면 알겠지만,
  결국 초록색 그래프가 의미하는 부분이 뭐냐면,

  '동일한 x좌표값을 공유하는 하나의 열 안에 존재하는 픽셀들의 배경색 색상값'
  을 그래프로 나타낸 거라고 보면 됨.

  즉, 하나의 열에 포함되는 각 픽셀들의 
  오로지 배경색으로써의 vec3 color의 값이
  각 열마다 어느 정도가 되는건지 그래프로 나타낸거임.

  그니까, 배경색 색상값이 (0, 0, 0)에 가까울수록
  그래프가 아래쪽에 위치하고, 배경색은 black에 가까운 반면,

  배경색 색상값이 (1, 1, 1)에 가까울수록
  그래프가 위쪽에 위치하고, 배경색은 white에 가까워지겠지

  7장 강의에서 나머지 예제들을 통해 내장함수를 설명하는 내용을 들을 때
  이 사실을 참고하면 내용을 이해하는 게 더 수월해질 것임.

  '초록색 그래프 = 하나의 열 안에 존재하는 픽셀들의 배경색 색상값'
*/